...
use value::{ValueID, ValueStore, ValueData, TypeStore, TypeID, ValueManager, ValueManagerRef};
...
impl<'a> Builder<'a> {
    ...
    pub fn number_constant(&mut self, v: i64) -> Result<ValueID, Error> {
        ...
        self.manager.try_borrow_mut()?.new_value_from_llvm(values::BasicValueEnum::IntValue(t.const_int(v.abs() as u64, v < 0)), t)
    }

    pub fn boolean_constant(&mut self, v: bool) -> Result<ValueID, Error> {
        let t = types::IntType::bool_type();
        self.manager.try_borrow_mut()?.new_value_from_llvm(values::BasicValueEnum::IntValue(t.const_int(v as u64, false)), t)
    }

    pub fn empty_constant(&self) -> Result<ValueID, Error> {
        self.manager.try_borrow()
            .map_err(Into::into)
    ...
    pub fn add(&mut self, lhs: ValueID, rhs: ValueID) -> Result<ValueID, Error> {
        ...
        let rhs_cl = self.manager.try_borrow()?.llvm_value(lhs)?;
        let res = self
            .inst_builder
            .build_int_add(lhs_cl.into_int_value(), rhs_cl.into_int_value(), "add");
        self.manager.try_borrow_mut()?.new_value_from_llvm(res, types::IntType::i64_type())
    }
    ...
    pub fn cmp(&mut self, cmp_type: CondCode, lhs: ValueID, rhs: ValueID) -> Result<ValueID, Error> {
        ...
        self.manager.try_borrow_mut()?.new_value_from_llvm(res, types::IntType::bool_type())
    }
    ...
    pub fn get_var(&mut self, name: &str) -> Result<Option<ValueID>, Error> {
        self.scope_stack.get_var(name).map_or(Ok(None), |var| {
            ...
            self.manager.try_borrow_mut()?.new_value_from_llvm(loaded, llvm_type).map(Some)
        })
    }
    ...
    pub fn cast_to(&mut self, v: ValueID, to_type: TypeID) -> Result<ValueID, Error> {
        ...
        let bool_type = self.manager.try_borrow()?.primitive_type(PrimitiveKind::Boolean);
    ...
    pub fn load(&mut self, t: TypeID, addr: values::PointerValue, offset: u32) -> Result<ValueID, Error> {
        ...
        let ptr = unsafe { self.inst_builder.build_in_bounds_gep(addr, &[types::IntType::i32_type().const_int(offset as u64, false)], "store") };
        let llvm_type = self.manager.try_borrow()?.llvm_type(t)?;
        self.manager.try_borrow_mut()?.new_value_from_llvm(self.inst_builder.build_load(ptr, "load"), llvm_type)
    }
    ...
    pub fn ret_int(&mut self, v: ValueID) -> Result<(), Error> {
        ...
        let cl = self.manager.try_borrow()?.llvm_value(return_value)?.into_int_value();
        self.inst_builder
