...
impl TypeInfer {
    ...
    fn bin_op(&self, op: Operator, lhs: &ir::Value, rhs: &ir::Value) -> Result<ir::Value, Error> {
        let new_inst = ir::Value::BinOp(op.clone(), box lhs.clone(), box rhs.clone());
...
impl Transform for TypeInfer {
    fn transform(&self, eir: &ir::Value) -> Result<ir::Value, Error> {
        match eir {
            ...
            ir::Value::BinOp(op, box lhs, box rhs) => self.bin_op(op.clone(), &self.transform(&lhs)?, &self.transform(&rhs)?),
            _ => unimplemented!()
        }
    }
}
